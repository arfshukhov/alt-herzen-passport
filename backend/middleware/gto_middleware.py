...
class GTOReader:
    ...
    def compare_results(self):
        if self.student_id:
            print(str(datetime.now().year)== datetime.now().year)
            print(datetime.now().year)
            res = BaseGTO.select().where(
    ...
    def get_all_members(self):
        gold = BaseGTO.select().where(
            BaseGTO.level == "gold"
        ).count()
        silver = BaseGTO.select().where(
            BaseGTO.level == "silver"
        ).count()
        bronze = BaseGTO.select().where(
...
class RatingByInstitute:
    def __init__(self, *,
        ...
        self.count = Institutes.select().count()
...
class GTOWriter:
    ...
    def update(self):
        for i in BaseGTO.select():
            if i.student_id == self.student_id:
                gto = BaseGTO.update(
                    {BaseGTO.level: self.level}
                ).where(
                    (BaseGTO.student_id == self.student_id) &
            ...
            raise NotFound(f"{self.student_id} not found.")
