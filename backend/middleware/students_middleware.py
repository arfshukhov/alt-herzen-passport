...
class StudentsReader:
    ...
    def __init__(self,*, _id:Union[int,None]=None,
        ...
        if self._id:
            ...
            """
            try:
                self.student = Students.select().where(Students.id == self._id).get()
            ...
            try:
                self.student = Students.select().where(
...
class StudentsList:
    def __init__(self, *,
        ...
        self.count = Students.select().count()
        if institute_id:
...
class StudentWriter:
    ...
    def write(self):
        Students(**self.data_to_update).save()
        user = StudentsReader(
    ...
    def update(self):
        q = Students.update(**self.data_to_update)\
                        .where(Students.id == self._id).execute()
        if q == 0:
    ...
    def delete(self):
        q = Students.delete() .where(Students.id == self._id).execute()
        if q == 0:
            raise KeyError("user not found")
